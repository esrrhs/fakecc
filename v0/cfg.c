...
void cfg_free(CFG *g) {
    for (size_t i = 0; i < g->num; i++) {
        runtime.free(g->blocks[i].preds);
        runtime.free(g->blocks[i].succs);
    }
    runtime.free(g->blocks);
    g->blocks = ((void*)0);
    g->num = 0;
}
int cfg_find_label(const CFG *g, int label) {
