...
static void build_interf_graph_cfg(const IRFunction *fn, const CFG *cfg,
    ...
    for (size_t bi = 0; bi < cfg->num; bi++) {
        ...
        for (size_t i = blk->end; i > blk->start; i--) {
            ...
            if (inst->op == IR_LABEL || inst->op == IR_BR) continue;
            if (inst->op == IR_CALL) {
                for (int _wi = 0; _wi < (&live)->num_words; _wi++) for (uint64_t _w = (&live)->w[_wi], over; _w && ((over = _wi * 64 + __builtin_ctzll(_w)), 1); _w &= _w - 1) {
    ...
    }
    {
        ...
        while (changed) {
            ...
            for (size_t bi = 0; bi < cfg->num; bi++) {
                const CFGBlock *blk = &cfg->blocks[bi];
                for (int _wi = 0; _wi < (&out_b[bi])->num_words; _wi++) for (uint64_t _w = (&out_b[bi])->w[_wi], v; _w && ((v = _wi * 64 + __builtin_ctzll(_w)), 1); _w &= _w - 1) {
                    for (size_t si = 0; si < blk->num_succs; si++) {
                        int s = blk->succs[si];
    ...
    for (size_t bi = 0; bi < cfg->num; bi++) {
        ...
        for (size_t i = blk->end; i > blk->start; i--) {
            ...
            if (inst->dst >= 0 && inst->dst < nv &&
                value_in_class(fn, inst->dst, float_class)) {
                for (int _wi = 0; _wi < (&live)->num_words; _wi++) for (uint64_t _w = (&live)->w[_wi], other; _w && ((other = _wi * 64 + __builtin_ctzll(_w)), 1); _w &= _w - 1) {
