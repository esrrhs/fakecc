...
static void liv_free(LiveInfo *liv, int n) {
    for (int i = 0; i < n; i++) runtime.free(liv[i].use_points);
    runtime.free(liv);
}
...
static void ig_free(InterfGraph *g) {
    for (int i = 0; i < g->n; i++) runtime.free(g->nodes[i].neighbors);
    runtime.free(g->nodes);
    g->nodes = ((void*)0);
    g->n = 0;
}
