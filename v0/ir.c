...
int g_no_builtin = 0;
static char *g_no_builtin_names[64];
static int g_n_no_builtin_names;
void ir_disable_builtin(const char *name) {
    if (!name || !*name) return;
    if (g_n_no_builtin_names >= 64) return;
    g_no_builtin_names[g_n_no_builtin_names++] = xstrdup(name);
}
int ir_builtin_disabled(const char *name) {
    if (!name) return 0;
    if (runtime.strncmp(name, "__builtin_", 10) == 0) return 0;
    for (int i = 0; i < g_n_no_builtin_names; i++) {
        if (runtime.strcmp(g_no_builtin_names[i], name) == 0) return 1;
    }
    return g_no_builtin;
}
const StructRegistry *get_ir_structs(void) {
...
struct VaArgPackFrame {
    ...
    Type ret_type;
    int is_void;
};typedef struct VaArgPackFrame VaArgPackFrame;
...
static int expr_has_setjmp(const Expr *e) {
    ...
    if (e->kind == EX_CALL && e->u.call.callee && e->u.call.callee->kind == EX_VAR &&
        runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_setjmp") == 0) return 1;
    switch (e->kind) {
...
static IRValue bool_normalize(IRFunction *fn, IRValue v, int w, int u,
    ...
    if (is_float) {
        ...
        return emit_fcmp(fn, v, zero, w, 5 , loc);
    }
    int cw = (w < 4) ? 4 : w;
    if (w > 0 && w != cw)
...
static IRValue emit_float_abs(IRFunction *fn, IRValue v, int elem_sz, SourceLoc loc) {
    ...
    IRValue lt0 = emit_fcmp(fn, v, zero, elem_sz, 0 , loc);
...
static const IRSlot *irsymtable_find(const IRSymTable *st, const char *name) {
    ...
    for (size_t i = g_ir_globals_st.len; i > 0; i--)
        if (runtime.strcmp(g_ir_globals_st.data[i-1].name, name) == 0) return &g_ir_globals_st.data[i-1];
    return ((void*)0);
}
...
struct BosRef {
    ...
    long long inner_off;
};typedef struct BosRef BosRef;
struct BosCtx {
    IRSymTable *st;
    const char *vis[24];
    int nvis;
    int bos_type;
};typedef struct BosCtx BosCtx;
...
static int bos_is_param(const char *name) {
    ...
    for (size_t i = 0; i < g_ir_cur_fd->params.len; i++) {
        if (g_ir_cur_fd->params.data[i].name &&
            runtime.strcmp(g_ir_cur_fd->params.data[i].name, name) == 0)
            return 1;
    }
    return 0;
}
static int bos_lvalue_is_name(const Expr *e, const char *name) {
    ...
    return e && e->kind == EX_VAR && e->u.var.name && name &&
           runtime.strcmp(e->u.var.name, name) == 0;
}
static int bos_is_memcpy_family(const char *n) {
    if (!n) return 0;
    if (runtime.strcmp(n, "memcpy") == 0 || runtime.strcmp(n, "memmove") == 0 ||
        runtime.strcmp(n, "mempcpy") == 0 || runtime.strcmp(n, "memset") == 0)
        return 1;
    if (runtime.strcmp(n, "__builtin_memcpy") == 0 || runtime.strcmp(n, "__builtin_memmove") == 0 ||
        runtime.strcmp(n, "__builtin_mempcpy") == 0 || runtime.strcmp(n, "__builtin_memset") == 0)
        return 1;
    if (runtime.strcmp(n, "__memcpy_chk") == 0 || runtime.strcmp(n, "__memmove_chk") == 0 ||
        runtime.strcmp(n, "__mempcpy_chk") == 0 || runtime.strcmp(n, "__memset_chk") == 0)
        return 1;
    if (runtime.strcmp(n, "__builtin___memcpy_chk") == 0 ||
...
static int bos_is_alloca(const char *n) {
    return n && (runtime.strcmp(n, "__builtin_alloca") == 0 || runtime.strcmp(n, "alloca") == 0);
}
static int bos_is_chk_builtin(const char *n) {
    return n && (runtime.strcmp(n, "__builtin___memcpy_chk") == 0 ||
                 ...
                 runtime.strcmp(n, "__builtin___strncpy_chk") == 0);
}
...
static int bos_is_stpcpy_chk_builtin(const char *n) {
    return n && (runtime.strcmp(n, "__builtin___stpcpy_chk") == 0 ||
                 runtime.strcmp(n, "__builtin___strcpy_chk") == 0);
}
...
static const char *bos_chk_plain_name(const char *n) {
    ...
    if (runtime.strstr(n, "strncpy")) return "strncpy";
    return "memcpy";
}
static const char *bos_chk_rt_name(const char *n) {
    if (runtime.strstr(n, "mempcpy")) return "__mempcpy_chk";
    ...
    if (runtime.strstr(n, "strncpy")) return "__strncpy_chk";
    return "__memcpy_chk";
}
...
static void bos_collect_stmt(const Stmt *s, const char *name,
    ...
    switch (s->kind) {
    case ST_DECL:
        if (s->u.decl.name && name && runtime.strcmp(s->u.decl.name, name) == 0 && s->u.decl.init) {
    ...
    if (!is_local && g_ir_tu) {
        for (size_t i = 0; i < g_ir_tu->globals.len; i++) {
            ...
            if (gs->kind == ST_DECL && gs->u.decl.name && name &&
                runtime.strcmp(gs->u.decl.name, name) == 0 && gs->u.decl.init) {
...
static int bos_vis_push(BosCtx *ctx, const char *name) {
    for (int i = 0; i < ctx->nvis; i++)
        if (ctx->vis[i] && name && runtime.strcmp(ctx->vis[i], name) == 0)
            return 0;
...
static BosRef bos_objref(BosCtx *ctx, const Expr *e) {
    ...
    if (e->kind == EX_MEMBER) {
        ...
        const StructDef *sd = g_ir_structs ? struct_registry_find_c(g_ir_structs, ot.tag) : ((void*)0);
    ...
    if (e->kind == EX_BINOP && (e->u.bin.op == BOP_ADD || e->u.bin.op == BOP_SUB)) {
        const Expr *ptr = ((void*)0), *idxe = ((void*)0);
        Type pty;
    ...
    if (e->kind == EX_VAR && e->u.var.name) {
        ...
        if (e->type.kind == TY_PTR) {
            ...
            for (int i = 0; i < ns; i++) {
                ...
                else if (n != n0 || o != o0 || runtime.memcmp(b, b0, (size_t)n) != 0) {
                    ok = 0;
                    break;
                }
            }
        ...
        for (size_t i = 0; i < g_ir_tu->globals.len; i++) {
            ...
            if (runtime.strcmp(gs->u.decl.name, e->u.var.name) != 0) continue;
...
IRValue out_i;
    ...
    if (bop == BOP_ADD) {
        ...
        if (is_float && elem_sz <= 8) {
            ...
            IRValue cond = emit_fcmp(fn, abs_i, abs_r, elem_sz, 1 , e->loc);
...
static const FunctionDecl *find_fn_with_body(const char *name) {
    const FunctionDecl *found = ((void*)0);
    if (!g_ir_tu || !name) return ((void*)0);
    for (size_t i = 0; i < g_ir_tu->functions.len; i++) {
        const FunctionDecl *fd = &g_ir_tu->functions.data[i];
        if (fd->name && runtime.strcmp(fd->name, name) == 0 && fd->body.len > 0)
            found = fd;
    }
    return found;
}
...
IRValue pr;
    ...
    case EX_CALL: {
        if (e->u.call.callee && e->u.call.callee->kind == EX_VAR
            ...
            if (runtime.strcmp(pack_cn, "__builtin_va_arg_pack_len") == 0) {
            ...
            if (runtime.strcmp(pack_cn, "__builtin_va_arg_pack") == 0) {
                IRValue v = new_value(fn);
            ...
            if (e->u.call.args.len > 0
                ...
                Expr **args = runtime.malloc((n ? n : 1) * sizeof(Expr *));
                if (!args) { runtime.fprintf(runtime.stderr, "fakecc: OOM\n"); runtime.exit(1); }
        ...
        if (e->u.call.callee->kind == EX_VAR) {
            ...
            if (runtime.strcmp(bos_cn, "__builtin_object_size") == 0 && e->u.call.args.len >= 1) {
        ...
        if (e->u.call.callee->kind == EX_VAR && e->u.call.args.len > 0) {
            ...
            if (runtime.strcmp(cn, "__builtin_crealf") == 0 || runtime.strcmp(cn, "__builtin_creal") == 0 ||
                ...
                int imag = runtime.strstr(cn, "cimag") != ((void*)0);
                Type cty = e->u.call.args.data[0]->type;
                int total_sz = type_size(cty);
                int elem_sz = total_sz / 2;
                int is_unsigned = (cty.tag && runtime.strstr(cty.tag, "unsigned") != ((void*)0));
        ...
        if (e->u.call.callee->kind == EX_VAR &&
            ...
            if (runtime.strcmp(sname, "__sync_bool_compare_and_swap") == 0 ||
                runtime.strcmp(sname, "__sync_val_compare_and_swap") == 0) {
                int val_sz = sz;
                int val_u = is_u;
                if (runtime.strcmp(sname, "__sync_bool_compare_and_swap") == 0) {
                    if (e->u.call.args.data[0]->type.kind == TY_PTR && e->u.call.args.data[0]->type.pointee) {
            ...
            if (runtime.strcmp(sname, "__sync_lock_test_and_set") == 0) {
            ...
            set_value_type(fn, old_val, sz, is_u);
            int op = IR_ADD;
            if (runtime.strstr(sname, "_sub_") || runtime.strstr(sname, "fetch_and_sub")) op = IR_SUB;
            else if (runtime.strstr(sname, "_and_and_") || runtime.strstr(sname, "fetch_and_and")) op = IR_BAND;
            else if (runtime.strstr(sname, "_or_and_") || runtime.strstr(sname, "fetch_and_or")) op = IR_BOR;
            else if (runtime.strstr(sname, "_xor_and_") || runtime.strstr(sname, "fetch_and_xor")) op = IR_BXOR;
            else if (runtime.strstr(sname, "_add_") || runtime.strstr(sname, "fetch_and_add")) op = IR_ADD;
            IRValue res = emit_bin_w(fn, op, old_val, delta, sz, is_u, e->loc);
            emit_inst_w(fn, IR_STORE_PTR, -1, addr, res, 0, sz, is_u, e->loc);
            if (runtime.strncmp(sname, "__sync_fetch_", 13) == 0) {
                return old_val;
            } else {
                return res;
            }
        ...
        if (e->u.call.callee->kind == EX_VAR && runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_stack_save") == 0) {
            IRValue v = new_value(fn);
        ...
        if (e->u.call.callee->kind == EX_VAR && runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_stack_restore") == 0) {
            IRValue ptr = lower_expr(fn, st, e->u.call.args.data[0]);
            emit_inst_w(fn, IR_STACK_RESTORE, -1, ptr, -1, 0, 8, 1, e->loc);
            fn->has_dyn_alloca = 1;
            return -1;
        }
        if (e->u.call.callee->kind == EX_VAR && runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_setjmp") == 0) {
        ...
        if (e->u.call.callee->kind == EX_VAR && runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_expect") == 0) {
            if (e->u.call.args.len >= 2) {
        ...
        if (e->u.call.callee->kind == EX_VAR && runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_longjmp") == 0) {
            IRValue buf_ptr = lower_expr(fn, st, e->u.call.args.data[0]);
        ...
        if (e->u.call.callee->kind == EX_VAR &&
            (runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_isinf") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_isinff") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_isinfl") == 0) &&
            e->u.call.args.len > 0) {
            Expr *a0 = e->u.call.args.data[0];
            Type aty = a0->type;
            if (aty.kind == TY_FLOAT) {
        ...
        if (e->u.call.callee->kind == EX_VAR &&
            (runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_add_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_add_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_sadd_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_sadd_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_saddl_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_saddl_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_saddll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_saddll_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uadd_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uadd_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uaddl_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uaddl_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uaddll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uaddll_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_sub_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_sub_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssub_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssub_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssubl_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssubl_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssubll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ssubll_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usub_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usub_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usubl_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usubl_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usubll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_usubll_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_mul_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_mul_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smul_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smul_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smull_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smull_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smulll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_smulll_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umul_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umul_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umull_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umull_overflow_p") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umulll_overflow") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umulll_overflow_p") == 0) &&
            e->u.call.args.len == 3) {
            const char *cname = e->u.call.callee->u.var.name;
            int is_add = runtime.strstr(cname, "add") != ((void*)0);
            int is_sub = runtime.strstr(cname, "sub") != ((void*)0);
            int is_p = runtime.strstr(cname, "_p") != ((void*)0);
...
IRValue hi;
        ...
        if (e->u.call.callee->kind == EX_VAR &&
            (runtime.strcmp(e->u.call.callee->u.var.name, "uabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "ulabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "ullabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "umaxabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_uabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ulabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_ullabs") == 0 ||
             runtime.strcmp(e->u.call.callee->u.var.name, "__builtin_umaxabs") == 0) &&
            e->u.call.args.len > 0 &&
            !ir_builtin_disabled(e->u.call.callee->u.var.name)) {
            const char *bname = e->u.call.callee->u.var.name;
            int bw = (runtime.strstr(bname, "ulabs") || runtime.strstr(bname, "ullabs") || runtime.strstr(bname, "umaxabs")) ? 8 : 4;
            IRValue arg = lower_expr(fn, st, e->u.call.args.data[0]);
            arg = coerce(fn, arg, get_value_width(fn, arg), get_value_is_unsigned(fn, arg), bw, 0, e->loc);
            IRValue slot = new_value(fn);
        ...
        if (e->u.call.callee->kind == EX_VAR) {
            ...
            if (runtime.strcmp(cname, "__builtin_shuffle") == 0) {
                Type vt = e->type;
                int total_sz = type_size(vt);
                int esz = vt.elem_type ? type_size(*vt.elem_type) : 4;
                if (esz <= 0) esz = 4;
        ...
        if (e->u.call.callee && e->u.call.callee->kind == EX_VAR) {
            ...
            if (runtime.strcmp(name, "__builtin_isnan") == 0 || runtime.strcmp(name, "__builtin_isnanf") == 0 ||
                runtime.strcmp(name, "__builtin_isnanl") == 0 || runtime.strcmp(name, "isnan") == 0) {
                IRValue arg = lower_expr(fn, st, e->u.call.args.data[0]);
                int w = get_value_width(fn, arg);
                return emit_fcmp(fn, arg, arg, w, 5 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isfinite") == 0 || runtime.strcmp(name, "__builtin_isfinitef") == 0 ||
                runtime.strcmp(name, "__builtin_isfinitel") == 0 || runtime.strcmp(name, "isfinite") == 0) {
                IRValue arg = lower_expr(fn, st, e->u.call.args.data[0]);
                int w = get_value_width(fn, arg);
                IRValue diff = emit_bin_w(fn, IR_FSUB, arg, arg, w, 0, e->loc);
                set_value_float(fn, diff, 1);
                IRValue zero = emit_float_const(fn, w, 0, e->loc);
                return emit_fcmp(fn, diff, zero, w, 4 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isinf") == 0 || runtime.strcmp(name, "__builtin_isinff") == 0 ||
                runtime.strcmp(name, "__builtin_isinfl") == 0 || runtime.strcmp(name, "isinf") == 0) {
                IRValue arg = lower_expr(fn, st, e->u.call.args.data[0]);
                int w = get_value_width(fn, arg);
                IRValue diff = emit_bin_w(fn, IR_FSUB, arg, arg, w, 0, e->loc);
                set_value_float(fn, diff, 1);
                IRValue zero = emit_float_const(fn, w, 0, e->loc);
                IRValue not_fin = emit_fcmp(fn, diff, zero, w, 5 , e->loc);
                IRValue not_nan = emit_fcmp(fn, arg, arg, w, 4 , e->loc);
                return emit_bin_w(fn, IR_BAND, not_fin, not_nan, 4, 0, e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isgreater") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int w = get_value_width(fn, a);
                return emit_fcmp(fn, a, b, w, 2 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isgreaterequal") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int w = get_value_width(fn, a);
                return emit_fcmp(fn, a, b, w, 3 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isless") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int w = get_value_width(fn, a);
                return emit_fcmp(fn, a, b, w, 0 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_islessequal") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int w = get_value_width(fn, a);
                return emit_fcmp(fn, a, b, w, 1 , e->loc);
            }
            if (runtime.strcmp(name, "__builtin_islessgreater") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int w = get_value_width(fn, a);
                IRValue lt = emit_fcmp(fn, a, b, w, 0 , e->loc);
                IRValue gt = emit_fcmp(fn, a, b, w, 2 , e->loc);
                return emit_bin_w(fn, IR_BOR, lt, gt, 4, 0, e->loc);
            }
            if (runtime.strcmp(name, "__builtin_isunordered") == 0) {
                IRValue a = lower_expr(fn, st, e->u.call.args.data[0]);
                IRValue b = lower_expr(fn, st, e->u.call.args.data[1]);
                int wa = get_value_width(fn, a);
                int wb = get_value_width(fn, b);
                IRValue nan_a = emit_fcmp(fn, a, a, wa, 5 , e->loc);
                IRValue nan_b = emit_fcmp(fn, b, b, wb, 5 , e->loc);
            ...
            if (runtime.strcmp(name, "__builtin_inf") == 0 || runtime.strcmp(name, "__builtin_inff") == 0 ||
                ...
                int64_t bits = 0;
                if (w == 4) { float f = (float)ld; runtime.memcpy(&bits, &f, sizeof(f)); }
                else { double d = (double)ld; runtime.memcpy(&bits, &d, sizeof(d)); }
                return emit_float_const(fn, w, bits, e->loc);
            }
        }
...
static int fold_const_float(const Expr *e, long double *out, const IRModule *ir) {
    ...
    if (e->kind == EX_VAR && runtime.strcmp(e->u.var.name, "__FLT_MAX__") == 0) {
        ...
        float f;
        runtime.memcpy(&f, &u, 4);
        *out = (long double)f;
        return 1;
    }
    if (e->kind == EX_CALL && e->u.call.callee && e->u.call.callee->kind == EX_VAR) {
        const char *name = e->u.call.callee->u.var.name;
        if (runtime.strcmp(name, "__builtin_inf") == 0 || runtime.strcmp(name, "__builtin_huge_val") == 0) {
            *out = __builtin_inf();
            return 1;
        }
        if (runtime.strcmp(name, "__builtin_inff") == 0 || runtime.strcmp(name, "__builtin_huge_valf") == 0) {
            *out = __builtin_inff();
            return 1;
        }
        if (runtime.strcmp(name, "__builtin_infl") == 0 || runtime.strcmp(name, "__builtin_huge_vall") == 0) {
            *out = __builtin_infl();
            return 1;
        }
        if (runtime.strcmp(name, "__builtin_nan") == 0) {
            const char *tag = "";
        ...
        if (runtime.strcmp(name, "__builtin_nanf") == 0) {
            const char *tag = "";
            if (e->u.call.args.len > 0 && e->u.call.args.data[0]->kind == EX_STR)
                tag = e->u.call.args.data[0]->u.str.bytes;
        ...
        if (runtime.strcmp(name, "__builtin_nanl") == 0) {
            const char *tag = "";
            if (e->u.call.args.len > 0 && e->u.call.args.data[0]->kind == EX_STR)
                tag = e->u.call.args.data[0]->u.str.bytes;
    ...
    if (e->kind == EX_INIT_LIST) {
        ...
        case TY_STRUCT: {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (!sm || ((sm->name == ((void*)0) || sm->name[0] == '\0') && sm->bit_width >= 0)) continue;
                if (sm->bit_width > 0) {
                    long long fv = 0;
...
void ir_generate(const TranslationUnit *tu, IRModule *ir, int pin_locals) {
    ...
    for (size_t i = 0; i < tu->functions.len; i++) {
        ...
        IRValue **param_ebs = ((void*)0);
        int *param_nreg = ((void*)0);
        if (nparams > 0) {
            param_ebs = runtime.calloc(nparams, sizeof(*param_ebs));
            param_nreg = runtime.calloc(nparams, sizeof(*param_nreg));
        ...
        for (size_t p = 0; p < fd->params.len; p++) {
            ...
            if (pty.kind == TY_STRUCT || pty.is_vector || type_is_pair16(pty)) {
                ...
                if (param_nreg[p] > 0) {
                    int n = param_nreg[p];
                    SysVRegClass *cls = runtime.malloc((size_t)n * sizeof(SysVRegClass));
                    if (!cls) { runtime.fprintf(runtime.stderr, "fakecc: OOM\n"); runtime.exit(1); }
                    if (type_is_pair16(pty)) {
                        cls[0] = SYSV_CLS_INTEGER;
                        cls[1] = SYSV_CLS_INTEGER;
                    } else {
        ...
        if (param_ebs) {
            for (size_t p = 0; p < nparams; p++) runtime.free(param_ebs[p]);
        ...
        g_ir_label_vla_id = ((void*)0);
        g_ir_label_vla_id_count = 0;
        g_ir_vla_sp_slots = ((void*)0);
        g_ir_vla_count = 0;
        ...
        if (g_ir_vla_sp_slots) {
            runtime.free(g_ir_vla_sp_slots);
            g_ir_vla_sp_slots = ((void*)0);
        }
        if (g_ir_label_vla_id) {
            runtime.free(g_ir_label_vla_id);
            g_ir_label_vla_id = ((void*)0);
        }
        g_ir_label_vla_id_count = 0;
        g_ir_vla_count = 0;
