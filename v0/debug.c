...
static uint32_t strtab_add(Strtab *s, const char *str) {
    ...
    d_bytes(&s->buf, str, runtime.strlen(str) + 1);
    return off;
}
void emit_module_add_dbg_line(EmitModule *m, const char *file, int line,
                              int col, size_t pc_off) {
    ...
    if (m->num_dbg_lines > 0) {
        ...
        if (last->line == line && last->col == col && last->file &&
            runtime.strcmp(last->file, file) == 0)
            return;
    }
...
static void debug_members_free(DebugMember *members, size_t n) {
    if (!members) return;
    for (size_t i = 0; i < n; i++) runtime.free(members[i].name);
...
void debug_var_release(DebugVar *v) {
    runtime.free(v->name);
    runtime.free(v->file);
    runtime.free(v->sym_name);
    runtime.free(v->type_name);
    debug_members_free(v->members, v->num_members);
    runtime.free(v->ranges);
    v->name = ((void*)0);
    v->file = ((void*)0);
    v->sym_name = ((void*)0);
    v->type_name = ((void*)0);
...
void debug_call_site_release(DebugCallSite *cs) {
    runtime.free(cs->callee_name);
    for (size_t i = 0; i < cs->num_params; i++)
        runtime.free(cs->params[i].value_expr);
    runtime.free(cs->params);
...
static void ser_str(Buffer *b, const char *s) {
    if (!s) s = "";
    uint32_t n = (uint32_t)runtime.strlen(s);
    d_u32(b, n);
    d_bytes(b, s, n);
}
...
static int deser_var(const unsigned char **p, const unsigned char *end, DebugVar *v) {
    ...
    if (v->type_name && v->type_name[0] == '\0') { runtime.free(v->type_name); v->type_name = ((void*)0); }
    if (*p + 4 > end) return -1;
...
int debug_deserialize(EmitModule *m, const unsigned char *data, size_t len) {
    ...
    if (len < 8 || runtime.memcmp(p, "FDBG", 4) != 0) return -1;
    p += 4;
    ...
    for (uint32_t i = 0; i < nlines; i++) {
        ...
        if (p + 16 > end) { runtime.free(file); return -1; }
    ...
    for (uint32_t i = 0; i < nfuncs; i++) {
        ...
        if (deser_str(&p, end, &file) < 0) { runtime.free(name); return -1; }
        if (p + 4 + 24 + 4 > end) { runtime.free(name); runtime.free(file); return -1; }
        int line = (int)rd32(&p);
        ...
        runtime.free(name); runtime.free(file);
        ...
        for (uint32_t j = 0; j < ncs; j++) {
            ...
            if (cs.callee_name && cs.callee_name[0] == '\0') {
                runtime.free(cs.callee_name); cs.callee_name = ((void*)0);
            }
...
static uint64_t resolve_sym_addr(const EmitModule *m, const char *name,
    ...
    for (size_t i = 0; i < m->num_syms; i++) {
        if (!m->syms[i].name || runtime.strcmp(m->syms[i].name, name) != 0) continue;
...
static int type_die_match(const TypeDie *td, DebugTypeTag tag, int width,
    ...
    return runtime.strcmp(td->type_name, type_name) == 0;
}
...
static uint32_t type_die_get(TypeDieCache *c, Buffer *info, DebugTypeTag tag,
    ...
    if (tag == DBG_TY_PTR) {
        ...
        d_bytes(info, nm, runtime.strlen(nm) + 1);
        d_u8(info, enc);
...
static uint32_t type_die_struct(TypeDieCache *c, Buffer *info,
    ...
    uint32_t off = (uint32_t)info->len;
    d_uleb(info, 6);
    if (tag_name) d_bytes(info, tag_name, runtime.strlen(tag_name) + 1);
    else d_u8(info, 0);
    d_u32(info, (uint32_t)(width > 0 ? width : 1));
    for (size_t i = 0; i < nmembers; i++) {
        const DebugMember *m = &members[i];
        if (m->bit_width > 0) {
            d_uleb(info, 14);
            if (m->name) d_bytes(info, m->name, runtime.strlen(m->name) + 1);
            else d_u8(info, 0);
            d_u32(info, mem_ty[i]);
...
static void emit_debug_line(const EmitModule *m, uint64_t text_base, Buffer *line) {
    ...
    for (size_t i = 0; i < nfiles; i++) {
        d_bytes(&hdr, files[i], runtime.strlen(files[i]) + 1);
    ...
    for (size_t i = 0; i < m->num_dbg_lines; i++) {
        ...
        int file_idx = 1;
        for (size_t j = 0; j < nfiles; j++)
            if (e->file && runtime.strcmp(files[j], e->file) == 0) { file_idx = (int)j + 1; break; }
...
void debug_emit_dwarf(const EmitModule *m, uint64_t text_base_vaddr,
    ...
    for (size_t fi = 0; fi < m->num_dbg_funcs; fi++) {
        ...
        for (size_t vi = 0; vi < f->num_vars; vi++) {
            ...
            if (free_refined) runtime.free(refined.ranges);
        }
        for (size_t ci = 0; ci < f->num_call_sites; ci++) {
            ...
            if (cs->callee_name) {
                for (size_t r = 0; r < nfrefs; r++) {
                    if (runtime.strcmp(frefs[r].name, cs->callee_name) == 0) {
                        origin = frefs[r].off;
                        break;
                    }
                }
