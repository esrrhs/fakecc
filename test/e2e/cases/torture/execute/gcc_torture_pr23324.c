// expect: 0
/* Complex by-value nested struct/union passing with bitfields.
 * Ported from GCC c-torture pr23324.c.  Tests that nested structs/unions
 * with bitfields, mixed integer/float members, and a flexible array member
 * are passed and accessed correctly by value. */
package main;

extern void abort(void);

static void A(int x) { if (!x) abort(); }
...
callee_af7 (float ap7, double bp7, union ct7 cp7, struct et7 dp7,
            union ft7 ep7, int fp7, struct gt7 gp7, long int *hp7,
            long long int ip7)
{
  A (vv7 == ap7);
  A (wv7 == bp7);
