...
/* Port of gcc.c-torture/execute/builtins/pr22237-1.c (+ pr22237-1-lib.c +
 * lib/main.c). Original abort/exit checks are kept. Local memcpy aborts on
 * overlap, matching the GCC test lib (so overlapping union members must be
 * copied via the intermediate struct, not by memcpy onto themselves). */
