// runtime.strncpy: copies up to n bytes; if src is shorter than n, pads the
...
int main() {
    ...
    i = 0;
    while (i < 8) { buf[i] = 'X'; i = i + 1; }
    r = runtime.strncpy(buf, "abcdefgh", 8);
    if (r != buf) return 4;
    ...
    i = 0;
    while (i < 8) { buf[i] = 'X'; i = i + 1; }
    r = runtime.strncpy(buf, "abcdefghij", 8);
    if (r != buf) return 6;
