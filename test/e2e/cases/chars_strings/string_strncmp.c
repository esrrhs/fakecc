// strncmp: like strcmp but stops after n bytes.  Must still stop at
...
int main() {
    ...
    if (strncmp("abc", "abd", 3) >= 0) return 2;
    if (strncmp("abd", "abc", 3) <= 0) return 3;
    /* n == 0 always equal */
    if (strncmp("abc", "xyz", 0) != 0) return 4;
    /* stop at \0 within window: both strings terminate at the same
       position inside the window -> equal via the \0 check */
    if (strncmp("ab", "ab", 3) != 0) return 5;
    /* window shorter than the difference -> equal */
    if (strncmp("abcdef", "abcxyz", 2) != 0) return 6;
    return 0;
}
