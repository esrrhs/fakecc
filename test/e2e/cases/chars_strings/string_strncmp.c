// runtime.strncmp: like runtime.strcmp but stops after n bytes.  Must still stop at
...
int main() {
    ...
    if (runtime.strncmp("abcdef", "abcxyz", 3) != 0) return 1;
    /* differ within the window */
    if (runtime.strncmp("abc", "abd", 3) >= 0) return 2;
    if (runtime.strncmp("abd", "abc", 3) <= 0) return 3;
    /* n == 0 always equal */
    if (runtime.strncmp("abc", "xyz", 0) != 0) return 4;
    /* stop at \0 within window: both strings terminate at the same
       position inside the window -> equal via the \0 check */
    if (runtime.strncmp("ab", "ab", 3) != 0) return 5;
    /* window shorter than the difference -> equal */
    if (runtime.strncmp("abcdef", "abcxyz", 2) != 0) return 6;
    return 0;
}
