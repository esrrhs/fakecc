// runtime.memcpy: copies n bytes from src to dst and returns dst.
...
int main() {
    ...
    char dst[6];
    /* poison dst so we know runtime.memcpy actually wrote each byte */
    dst[0] = 'X'; dst[1] = 'X'; dst[2] = 'X'; dst[3] = 'X'; dst[4] = 'X'; dst[5] = 'X';
    char *r = (char *)runtime.memcpy(dst, src, 6);
    if (r != dst) return 1;
    if (dst[0] != 'H') return 2;
    if (dst[1] != 'e') return 3;
    if (dst[2] != 'l') return 4;
