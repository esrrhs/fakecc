...
/* Port of gcc.c-torture/execute/builtins/strncat-chk.c (+ chk.h + lib/chk.c +
 * lib/main.c). chk.h strncat/strcpy/memset macros are expanded to
 * __builtin___*_chk(..., __builtin_object_size(dst, 0)). GCC lib
 * inside_main/__OPTIMIZE__ abort-on-strncat/strcat is omitted
...
char *
__strncat_chk (char *d, const char *s, size_t n, size_t size)
{
  size_t len = strlen (d), n1 = n;
  const char *s1 = s;
