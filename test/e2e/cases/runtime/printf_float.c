// rt/printf.c's floating-point conversions, plus the flags and width handling
...
int main() {
    ...
    printf("%e|%E|%g|%g|%g|%g\n", 1234.5678, 0.000123, 0.0001, 100000.0,
           1234567.0, 1.5);
    printf("[%10.2f][%-10.2f][%010.2f][%+.1f][% .1f]\n",
           3.14159, 3.14159, 3.14159, 3.14159, 3.14159);
    printf("[%+d][% d][%05d][%-6d][%.5d][%#x][%5s][%-5s][%.2s]\n",
    ...
    char buf[64];
    int n = snprintf(buf, sizeof buf, "%.2f", 12.345);
    printf("%.0f|%.1f|%s\n", 2.5, 2.25, buf);
    if (n != 5) return 1;
    return 42;
}
