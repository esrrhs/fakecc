// runtime/printf.c's floating-point conversions, plus the flags and width handling
...
import runtime;
int main() {
    runtime.printf("%f|%f|%f|%.3f|%f\n", 3.75, -0.5, 1.0 / 3.0, 1234.56789, 0.0);
    runtime.printf("%e|%E|%g|%g|%g|%g\n", 1234.5678, 0.000123, 0.0001, 100000.0,
           1234567.0, 1.5);
    runtime.printf("[%10.2f][%-10.2f][%010.2f][%+.1f][% .1f]\n",
           3.14159, 3.14159, 3.14159, 3.14159, 3.14159);
    runtime.printf("[%+d][% d][%05d][%-6d][%.5d][%#x][%5s][%-5s][%.2s]\n",
           42, 42, 42, 42, 42, 255, "ab", "ab", "abcd");
    runtime.printf("[%*d][%-*d][%.*f]\n", 6, 7, 6, 7, 3, 2.5);
    ...
    char buf[64];
    int n = runtime.snprintf(buf, sizeof buf, "%.2f", 12.345);
    runtime.printf("%.0f|%.1f|%s\n", 2.5, 2.25, buf);
    if (n != 5) return 1;
    return 42;
}
