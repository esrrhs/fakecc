#!/bin/bash
# Build and run the tiny-bignum-c FakeCC port test.
...
OPT="${2:--O1}"
if [ ! -x "${FAKECC}" ]; then
    echo "FAIL tinybn: fakecc not found at ${FAKECC}"
    exit 1
fi

SRC="${SCRIPT_DIR}/../tinybn.c"
TEST_SRC="${SCRIPT_DIR}/test_tinybn.c"
# Unique output so app_ports_O0 and app_ports_O1 can run in parallel.
SAFE_OPT="${OPT#-}"
OUTPUT="${SCRIPT_DIR}/test_tinybn.${SAFE_OPT}.$$.bin"
trap 'rm -f "${OUTPUT}"' EXIT

if ! "${FAKECC}" "${OPT}" "${SRC}" "${TEST_SRC}" -o "${OUTPUT}"; then
    echo "FAIL tinybn: compilation failed"
    exit 1
fi
...
if "${OUTPUT}"; then
    ...
    echo "FAIL tinybn: program exited non-zero"
    exit 1
fi
