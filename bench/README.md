...
32 位 ALU / 立即数 / 就地计算 / 调用参数直传之后（同一套 case，绝对时间受机器负载影响，看比值）：

| 程序 | fakecc -O0 | gcc -O0 | O0 比值 | fakecc -O1 | gcc -O1 | O1 比值 | gcc -O2 |
|---|---:|---:|---:|---:|---:|---:|---:|
| nbody | 436.8 ms | 302.1 ms | 1.45× | 429.2 ms | 172.0 ms | 2.50× | 169.1 ms |
| sieve | 290.3 ms | 174.1 ms | 1.67× | 221.4 ms | 38.2 ms | 5.80× | 32.9 ms |
| matmul | 625.0 ms | 521.3 ms | 1.20× | 661.5 ms | 90.6 ms | 7.30× | 42.3 ms |
| chacha | 400.2 ms | 313.5 ms | 1.28× | 502.7 ms | 49.0 ms | 10.26× | 50.7 ms |

几何平均：**-O0 约 1.39×，-O1 约 5.7×**（>1 表示 fakecc 编出来的二进制更慢）。  
最初同一套 case 是 **-O0 ~2.0×，-O1 ~7.1×**；栈寻址折叠 + `rol` 之后是 **-O0 ~1.5×**。
...
3. **已经做的 `-O0` 指令选择（gcc `-O0` 也会做的那种，不是开优化）：**
   ...
   收口后 `-O0` 几何平均大约 **2.0× → 1.39×**（矩阵乘 1.20×，ChaCha 1.28×）。
...
5. **下一步若要收口 `-O1` 比值，优先整型循环：** 嵌套循环的地址计算收进 `[base+index*scale]`、循环展开 / 向量化。标量浮点（n-body 这一类）不是最急的。剩余的 `-O0` 零头主要是 RA 多出来的 `mov` 中转、全局数组仍走 `lea`+`add`+间接访问（`[base+index]` 要等寄存器活区间覆盖到 load/store 才能安全折叠）。
